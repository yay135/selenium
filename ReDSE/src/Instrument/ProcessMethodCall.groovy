...
class ProcessMethodCall {
	...
	String thisMethodBod = "";
	
	String tail  = "";
	
	
	
	public ProcessMethodCall(def exp , ArrayList<String> listForDef, ArrayList<ArrayList<String>> listForInput, ArrayList<String> listEnv, HashMap<String, String> mapForDefined, String declareVariable, ArrayList<String> listForLazy, ArrayList<String> runtimeDataBase) 
	{
		...
		arguments.each { a ->
			...
			ProcessExpression expression = new ProcessExpression(a, listForDef, listForInput, listEnv, mapForDefined, declareVariable, listForLazy, runtimeDataBase);
			
			String extraHead = expression.getHead();
			String extraTail = expression.getTail();
			
			head = head + extraHead + "\n";
			tail = tail + extraTail + "\n";
			
			String arg = expression.getExpression();
		...
		ProcessExpression objectExpression = new ProcessExpression(object, listForDef, listForInput, listEnv, mapForDefined, declareVariable, listForLazy, runtimeDataBase);
		ProcessExpression methodExpression = new ProcessExpression(method, listForDef, listForInput, listEnv, mapForDefined, declareVariable, listForLazy, runtimeDataBase);
		
		String extraHead = objectExpression.getHead();
		String extraTail = objectExpression.getTail();
		
		head = head + extraHead + "\n";
		tail = tail + extraTail + "\n";
		
		extraHead = methodExpression.getHead();
		extraTail = methodExpression.getTail();
		
		head = head + extraHead + "\n";
		tail = tail + extraTail + "\n";
		
		String objectStr = objectExpression.getExpression();
		String methodStr = methodExpression.getExpression();
		...
		if(objectStr == "this")
		{
			...
			thisMethodBod = thisMethodBod + str;
		}
		else 
		{
			int column = object.columnNumber;
			...
			String name = "tempVariable_" + column + line + method.columnNumber + method.lineNumber
